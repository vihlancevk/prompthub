...
func main() {
	...
	// Print the help message and exit if --help (-h) is passed
	if *help {
		pflag.PrintDefaults()
		os.Exit(0)
	}
	...
	if err := index.Init(viper.GetString("database_url")); err != nil {
		output.FATAL.Println("Failed to connect to database:", err)
		os.Exit(1)
	}
